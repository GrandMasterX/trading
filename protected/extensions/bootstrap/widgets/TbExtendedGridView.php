...
class TbExtendedGridView extends TbGridView
{
    ...
    /**
     * @var string the position of the bulk actions. It can be 'top', 'bottom' or 'both'.
     */
    public $bulkActionPosition = 'both';    
    
    /**
     * @var string class name for bulk action buttons holder. Is used for sticky plugin.
     */    
    public $bulkActionHtmlClass = 'bulk-button-holder';
    
    /**
     * @var bool fixedBulkActionPosition if set to true will keep the bulkAction buttons set at fixed position
     */
    public $fixedBulkActionPosition = true;

    /**
     * @var integer $bulkActionPositionOffset, when $fixedBulkActionPosition is set to true, $bulkActionPositionOffset will position the bulkAction buttons set to the top
     * at $bulkActionPositionOffset. If you are using bootstrap and has navigation top fixed, its height is 40px, so it is recommended
     * to use $bulkActionPositionOffset=40;
     */
    public $bulkActionPositionOffset = 0;    
	...
	public function init()
	{
		...
		if ($this->bulkActions !== array() && isset($this->bulkActions['actionButtons']))
		{
            ...
            $this->bulkActions['htmlClass'] = $this->bulkActionHtmlClass;
            $this->bulkActions['position'] = $this->bulkActionPosition;
            $this->bulkActions['bulkActionPositionOffset'] = $this->bulkActionPositionOffset;    
...
//	}
    ...
    public function renderTableHeader()
    {
        ...
        if(!$this->hideHeader)
        {
            ...
            if ($this->bulk !== null && $this->bulkActionPosition=='top' || $this->bulkActionPosition=='both')
	...
	public function renderTableFooter()
	{
		...
		if ($this->bulk !== null || $hasFilter || $hasFooter)
		{
			...
			if ($this->bulk !== null && $this->bulkActionPosition=='bottom' || $this->bulkActionPosition=='both')
				$this->renderBulkActions();
			echo "</tfoot>\n";
		}
	}
	...
	public function registerCustomClientScript()
	{
        ...
        //only if bulkActionPosition='top';
        $fixedTbBulkActionButtonsJs = '';
        if ($this->bulkActionPosition=='top')
        {
            Yii::app()->bootstrap->registerAssetJs('jquery.sticky.js');
            $fixedTbBulkActionButtonsJs = "$('#{$this->bulkActionHtmlClass}').sticky({topSpacing:{$this->bulkActionPositionOffset}});";
            $this->componentsAfterAjaxUpdate[] = $fixedTbBulkActionButtonsJs;
        }        
